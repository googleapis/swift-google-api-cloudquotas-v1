...
func sample(
  client: CloudQuotasClient, projectId: String, locationId: String, serviceId: String,
  quotaInfoId: String
) async throws {
  let response = try await client.getQuotaInfo(
    request: GetQuotaInfoRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/services/\(serviceId)/quotaInfos/\(quotaInfoId)"
      }
  )
